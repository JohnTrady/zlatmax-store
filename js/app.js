(() => {
    ...
    (() => {
        ...
        function rangeInit() {
            ...
            if (rangeItems.length) rangeItems.forEach(((rangeItem, index) => {
                const fromValue = document.querySelectorAll("[data-range-from]");
                const toValue = document.querySelectorAll("[data-range-to]");
                const item = document.querySelectorAll("[data-range-item]");
                nouislider.create(item[index], {
                    start: [ Number(fromValue[index].value), Number(toValue[index].value) ],
                    connect: true,
                    tooltips: [ true, true ],
                    format: wNumb({
                        decimals: 0
                    ...
                    range: {
                        min: [ Number(fromValue[index].dataset.rangeFrom) ],
                        max: [ Number(toValue[index].dataset.rangeTo) ]
                    }
                });
                item[index].noUiSlider.on("update", (function(values, handle) {
                    fromValue[index].value = values[0];
                    toValue[index].value = values[1];
                }));
                fromValue[index].addEventListener("change", setPriceValues);
                toValue[index].addEventListener("change", setPriceValues);
                function setPriceValues() {
                    let priceStartValue;
                    let priceEndValue;
                    if (fromValue.value != "") priceStartValue = fromValue[index].value;
                    if (toValue.value != "") priceEndValue = toValue[index].value;
                    item[index].noUiSlider.set([ priceStartValue, priceEndValue ]);
                }
            }));
        }
        rangeInit();
        ...
        if (document.querySelector("[data-tippy-content-bonuses]")) flsModules.tippy = tippy_esm("[data-tippy-content-bonuses]", {
            content: document.querySelector("[data-tippy-content-bonuses]").getAttribute("data-tippy-content-bonuses"),
            theme: "white",
            arrow: tippy_esm.roundArrow
        });
            ...
            if (document.querySelector(".main__slider")) new core(".main__slider", {
                ...
                on: {
                    init: function(swiper) {
                        const totalSlides = document.querySelector(".fraction-control__all");
        ...
        DynamicAdapt.prototype.init = function() {
            if (document.querySelector(".product-info__tabs")) {
                const tabsToSpoilers = () => {
                    if (document.querySelector(".product-info__spollers")) {
                        const tabsItems = document.querySelectorAll("[data-da-body]");
                        const spoilersItems = document.querySelectorAll("[data-spoiler-content ]");
                        const tabsItemTitles = document.querySelectorAll("[data-tabs-title]");
                        const spoilersItemsButtons = document.querySelectorAll("[data-spoiler-button]");
                        tabsItems.forEach(((tabItem, index) => {
                            spoilersItemsButtons[index].innerHTML = tabsItemTitles[index].innerHTML;
                            spoilersItems[index].classList.add(`tab-to-spoiler_${index + 1}`);
                            tabsItems[index].setAttribute("data-da", `.tab-to-spoiler_${index + 1},767.98`);
                        }));
                    }
                };
                tabsToSpoilers();
        ...
        const documentActions = e => {
            ...
            const activeLink = document.querySelector("._sub-menu__active");
            const activeBlock = document.querySelector("._sub-menu__open");
            const phonesBlock = document.querySelector(".phones-header__items");
            ...
            if (targetElement.closest("[data-parent]")) {
                ...
                if (subMenu) {
                    if (activeLink && activeLink !== targetElement) {
                        activeLink.classList.remove("_sub-menu__active");
                        activeBlock.classList.remove("_sub-menu__open");
                    }
                    targetElement.classList.toggle("_sub-menu__active");
                    subMenu.classList.toggle("_sub-menu__open");
                }
                e.preventDefault();
            } else if (!targetElement.closest(".catalog-header")) if (activeLink && activeBlock) {
                activeLink.classList.remove("_sub-menu__active");
                activeBlock.classList.remove("_sub-menu__open");
            }
            if (targetElement.closest(".phones-header__icon")) {
                if (phonesBlock) phonesBlock.classList.toggle("_active");
            } else if (!targetElement.closest(".phones-header__items") && !targetElement.closest(".phones-header__icon")) if (phonesBlock) phonesBlock.classList.remove("_active");
            if (targetElement.closest(".catalog-menu-top-header")) {
                document.documentElement.classList.add("_catalog-open");
                e.preventDefault();
            } else if (targetElement.closest(".icon-menu")) document.documentElement.classList.remove("_catalog-open");
            if (targetElement.closest(".menu-catalog__backspace")) {
                if (document.querySelector("._sub-menu__open")) {
                    document.querySelector("._sub-menu__open") ? document.querySelector("._sub-menu__open").classList.remove("_sub-menu__open") : null;
                    document.querySelector("._sub-menu__active") ? document.querySelector("._sub-menu__active").classList.remove("_sub-menu__active") : null;
                } else if (document.documentElement.classList.contains("_catalog-open")) {
                    document.documentElement.classList.remove("_catalog-open");
                    document.querySelector("._sub-menu__active") ? document.querySelector("._sub-menu__active").classList.remove("_sub-menu__active") : null;
                }
                e.preventDefault();
            }
            if (targetElement.closest("[data-filter-button]")) {
        ...
        const scrollActions = () => {
            if (pageYOffset >= 140) {
                if (document.querySelector(".js-catalog-header")) document.querySelector(".js-catalog-header").classList.add("catalog-header-sticky");
            } else if (document.querySelector(".js-catalog-header")) document.querySelector(".js-catalog-header").classList.remove("catalog-header-sticky");
        };
        if (document.querySelector(".flashlights-block")) {
            const flashlightsButtons = document.querySelectorAll(".js-flashlights-button");
            const flashlightsButtonsMobile = document.querySelectorAll(".js-flashlights-button-mobile");
            flashlightsButtons.forEach(((flashlightsButton, index) => {
                flashlightsButtonsMobile[index].innerHTML = flashlightsButton.innerHTML;
            }));
        }
        const getCheckboxItemId = () => {
            const ItemCheckboxes = document.querySelectorAll(".filter-item__checkbox");
            const ItemLabeles = document.querySelectorAll(".filter-item__label");
        ...
        if (document.querySelector(".reviews__items")) {
            const checkReviews = () => {
                const reviewItems = document.querySelectorAll(".item-review");
                const noReviewMessage = document.querySelector(".no-reviews");
                if (!reviewItems.length) noReviewMessage.classList.add("_show-message"); else noReviewMessage.classList.remove("_show-message");
            };
            checkReviews();
        }
